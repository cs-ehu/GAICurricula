...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2"
    ...
    <sch:pattern id="DatosDiferentes">
        ...
        <sch:rule context="curriculums">
            ...
            <sch:assert test="count(./curriculum/mobile) = count(distinct-values(./curriculum/mobile))" diagnostics="MovRep">
    ...
    <sch:pattern id="AñosCarreraIncorrectos">
        ...
        <sch:rule context="curriculums/curriculum">
            <sch:assert test="./carriere/@course-curso-cours &gt;= 2" diagnostics="DosCur">
                El alumno <sch:value-of select="./@name-nombre-nom"/> no ha cursado años suficientes.
            </sch:assert>
            
            <sch:assert test="./carriere/@course-curso-cours &lt;= 5" diagnostics="CincoCur">
                El alumno <sch:value-of select="./@name-nombre-nom"/> no puede tener un curso mayor a 5.
            </sch:assert>
            
            <sch:assert test="./carriere/@yearsstudying-añoscursando-anneesdetudes &gt;= ./carriere/@course-curso-cours" diagnostics="CursandoCursoIn">
                El alumno <sch:value-of select="./@name-nombre-nom"/> no puede estar cursando menos años que el curso en el que se encuentra.
            </sch:assert>
        </sch:rule>
        
        <sch:rule context="curriculums/curriculum/carriere[@course-curso-cours = 5]">
            <sch:assert test="contains(., 'Architecture') or contains(., 'Médecine')" diagnostics="InCur">
    ...
    <sch:pattern id="LenguaMaternaDiferenteALenguaExt">
        ...
        <sch:rule context="curriculums/curriculum/languesetrangeres/lng">
            <sch:assert test="contains(., ../../languematernelle/data()) = false()" diagnostics="IdiomaIn">
    ...
    <sch:pattern id="LenguasExtranjeras">
        ...
        <sch:rule context="curriculums/curriculum/languesetrangeres">
        ...
        <sch:rule context="curriculums/curriculum">
            <sch:assert test="count(.//lng[./data() = 'Spanish']) = 1" diagnostics="IdiomaEsp">
                El alumno <sch:value-of select="../@name-nombre-nom"/> debe saber castellano para hacer el erasmus en España.
            </sch:assert>
        </sch:rule>
        <sch:rule context="curriculums/curriculum/languesetrangeres/lng[./data() = 'Spanish']">
